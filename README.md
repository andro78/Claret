...
**Debug 빌드에서 `Ctrl+Shift+D`를 누르면 그 패널의 WebView2 개발자 도구가 열린다.** 이 경로가
없으면 페이지 콘솔에 닿을 방법이 아예 없다 — F12는 `AreBrowserAcceleratorKeysEnabled`와 함께
꺼져 있고(브라우저가 터미널 키를 훔쳐가지 않게 하려고), 우클릭은 붙여넣기가 쓴다. Release
빌드는 이 명령을 무시한다.

`terminal.html`은 `post('e' + 메시지)`로 자기 쪽 오류를 호스트에 보낸다 — WebGL 애드온을 못
불렀거나 테마·하이라이트 JSON이 깨졌을 때다. 호스트는 그걸 터미널에 흐린 알림으로 찍는다.
(오랫동안 호스트에 이 태그를 받는 분기가 없어서 전부 조용히 버려졌다. 새 태그를 페이지에
추가할 때는 `OnWebMessageReceived`의 `switch`에 짝을 함께 넣을 것.)

## 릴리스 전에 확인할 것

### 한글 모드에서 단축키를 한 번 훑는다

이건 형식적인 절차가 아니다. **한/영을 한글로 두면 페이지가 가진 단축키가 전부 죽어 있던
시기가 있었고**, 오래도록 아무도 몰랐다. 원인은 키 핸들러 첫 줄의 `keyCode === 229` 한 개였다
(`Assets/xterm/terminal.html`, `chordKey`).

한국어 사용자를 위한 터미널에서 **IME 모드는 사실상 두 번째 실행 환경이다.** 영문 모드에서
전부 통과해도 한글 모드에서 통째로 막힐 수 있고, 그 반대도 성립한다. 그러니 두 모드에서
각각 본다.

| 단축키 | 확인할 것 |
|---|---|
| `Ctrl+Shift+C` / `V` | 복사 / 붙여넣기 |
| `Ctrl+C` / `V` | 선택이 없을 때 `Ctrl+C`는 SIGINT로 가야 한다 |
| `Ctrl+Tab` / `Ctrl+Shift+Tab` | 세션 전환 |
| `Ctrl+Shift+T` / `W` / `B` | 새 탭 / 세션 닫기 / 사이드바 |
| `Alt+Shift++` / `-` | 패널 좌우 / 위아래 펼치기 |
| `Ctrl+=` / `-` / `0` | 글꼴 크기 |

한글 입력 자체도 함께 본다 — 조합, 확정, 두 배 폭 정렬.

### 클립보드는 앱을 닫은 뒤에 확인한다

복사한 다음 **Claret을 닫고** 다른 앱에 붙여 본다. `Clipboard.Flush()`가 빠지면 복사한 내용이
프로세스와 함께 사라지는데, 앱이 열려 있는 동안에는 멀쩡해 보여서 이 순서로만 드러난다.
